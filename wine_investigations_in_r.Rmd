...
#     geom_density() +
   ...
   p3 <- ggplot(data=mydata, aes( x = 1, y = Value ) ) + 
               geom_jitter(alpha = 0.1 ) +
               geom_boxplot(alpha = 0.2, color = 'red' ) +
               ggtitle(paste(name, ' distribution box plot')) +
               coord_flip()
           
plot_grid(p2, p3, p1, align = "v", nrow = 3, rel_heights = c(.3, .3, .4))
...
The distribution is normal and unimodal, but with a fatter tail on the high side. (Possibly log-normal?)

```{r boxplot_fixed_acidity, echo=FALSE, fig.height=8}
...
```{r boxplot_volatile_acidity, echo=FALSE, fig.height=8}
...
```{r boxplot_citric_acid, echo=FALSE, fig.height=8}  
...
```{r boxplot_residual_sugar, echo=FALSE, fig.height=8} 
...
```{r boxplot_chlorides, echo=FALSE, fig.height=8}  
...
```{r boxplot_free_sulfur_dioxide, echo=FALSE, fig.height=8}  
...
```{r boxplot_total_sulfur_dioxide, echo=FALSE, fig.height=8}  
...
```{r boxplot_density, echo=FALSE, fig.height=8}  
...
```{r boxplot_pH, echo=FALSE, fig.height=8}  
...
```{r boxplot_sulphates, echo=FALSE, fig.height=8}  
...
```{r boxplot_alcohol, echo=FALSE, fig.height=8}  
...
#### Interesting bivariate relationships
...
```{r free_sulfur_dioxide_v_volatile_acidity, echo=FALSE}
...
From the PCA data we see that most of the variance in the wine data is general 
acidity.  

```{r first_pa_components2}
  wine.pca$rotation[,1]
```


And although the median acidity goes up with wine quality, it it 
clear from the graph that there are wines with different levels of acidity in 
each quality bin.  This doesn't seem to be a strong differentiator.

```{r first_pa_components3}
  lm(first_pa~wine$quality)
```
...
ggplot(aes(factor(quality), 
  ...
  xlab("Quality") +
  ylab("First PA") +
  ggtitle("First PA (axis of acidity) vs. Quality")
```


But the next two principal components (PA2 and PA3) look like signatures of 
problems in winemaking. PA3, with its high volatile acidity and low alcohol, 
is probably related to wine going to vineagar. 

```{r third_pa_components2}  
  third_pa <- wine.pca$x[, 3]
  wine.pca$rotation[,3]
  lm(third_pa~wine$quality)
...
ggplot(aes(factor(quality), 
  ...
  xlab("Quality") +
  ylab("Third PA") +
  ggtitle("Third PA (axis of vinegar) vs. Quality")
```

PA2 has some volatile acidity and low alcohol, but is mainly distinguished
by a high total and free sulfur dioxide.  This can also lend an off-taste to wine.

```{r second_pa_components2}
  wine.pca$rotation[,2]
  second_pa <- wine.pca$x[, 2]
  lm(second_pa~wine$quality)
...
```{r second_pa2, echo=FALSE}
  ...
  xlab("Quality") +
  ylab("Second PA") +
  ggtitle("Second PA (axis of funk) vs. Quality")
...
Tolstoy said that all happy families are alike, but each unhappy family is 
unhappy in its own way.  Perhaps not true for families, but true enough for
wine, at least for the wine in the top bins vs. the wines in the middle and 
lower bins.

What distinguishes the higher quality wine is the absence of wine faults.
It is fairly easy to distinguish the best wine from the others by the absence 
of these faults.  PCS/LCA analysis shows some possible combinations of features 
that could be a signature of wine faults, but of course it's just exploratory.  

As far as the wines in the middle quality bins  (that is, the overwhelming
majority of the wine samples), the picture becomes more hazy because there are
many types of wine fault, so wines can be less-than-perfect in many different 
ways, in different degrees and different combinations.  I'm not sure if the 
mid-quality wines are blended, in which case, you'd expect wines with
complementary faults to be mixed together, which would muddy the water still 
more.
